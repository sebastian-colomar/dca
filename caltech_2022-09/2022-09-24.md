...
1. We can alternatively use the option `publish-all` instead of `publish`:
    ...
    ```
1. The exposed container port will be publish in a random port of the host machine starting with number `32768`. We can check that randomly chosen port with the command:
