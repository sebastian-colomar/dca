...
1. Docker compose will be equivalent to running the following Docker command:
    ...
    ```
    sudo docker run --volume ${HOME}/anagrams/anagrams.py:/app/script.py:ro --volume ${HOME}/anagrams/words.txt:/app/words.txt:ro --workdir /app/ index.docker.io/library/python:alpine python3 script.py
    ```
