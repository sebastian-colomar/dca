...
```
The section Graph Driver of the previous output shows the actual location inside the host filesystem of the image layers. The Lower Directory contains the superposition of the immutable lower layers in reverse order. The last folder in the Lower Directory sections contains the first layer of the Docker image corresponding to the first line of the Docker file (that is the Alpine operating system base image). That first layer does not contain the following binaries: curl, xz, openssl. These packages were installed by the second line of the Dockerfile and they are therefore contained inside the second layer of the Lower Directory as can be checked with the following commands:
```
labsuser@ip-172-31-13-145:~$ sudo find /var/lib/docker/overlay2/f7a83dc729b7d554ce1c1e2ff567407720377f177f49dbb704a47c6343a0cc2f/diff | grep -E "bin/curl$|bin/xz$|bin/openssl$"
labsuser@ip-172-31-13-145:~$ sudo find /var/lib/docker/overlay2/fe353517eea371162ce8cb8b25dbb3966b77043cca4c761268be28fbc1fb1795/diff | grep -E "bin/curl$|bin/xz$|bin/openssl$"
