...
```
1. Docker container: isolated environment created using Linux namespaces and control groups (isolated partitions of the kernel resources of the host machine)
2. Docker image: external root filesystem containing a basic operating system and the necessary libraries and dependencies
3. Docker volume: external filesystem containing the custom data and configuration for our Docker container
4. Docker network: network that will provide a valid IP address to our Docker container
