...
```
The host prefix:
* The subnet prefix length to assign to each individual node. For example, if hostPrefix is set to 23 then each node is assigned a /23 subnet out of the given cidr. A hostPrefix default value of 24 provides 254 (2^(32 - 24) - 2) pod IP addresses.
```
mkdir -p ${HOME}/.kube
