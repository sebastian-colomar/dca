...
If you have 100 worker nodes that will do:
```
100 worker x 100 pod/worker = 10k pods
```
The host prefix:
* The subnet prefix length to assign to each individual node. For example, if hostPrefix is set to 24 then each node is assigned a /24 subnet out of the given cidr. A hostPrefix default value of 24 provides 254 (2^(32 - 24) - 2) pod IP addresses.
