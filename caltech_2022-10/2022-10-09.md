...
1. Run the following command on one of the workers:
    ...
    ```
    sudo docker run --detach --name registry --publish 80:5000 --restart always --user root:root --volume registry_volume:/var/lib/registry/:rw index.docker.io/library/registry:2
    ```
