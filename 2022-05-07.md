...
sudo docker service create --entrypoint php --name phpinfo-config --publish 8080 --read-only --replicas 2 --restart-condition any --config source=index.php,target=/app/index.php,mode=0400,uid=65534 --user nobody --workdir /app/ php -f index.php -S 0.0.0.0:8080
```
